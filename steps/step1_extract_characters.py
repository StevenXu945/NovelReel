...
3. 判断每个本章节出场角色的视觉资产动作：
   ...
   - "new": 全局资产里没有的新增角色，或同一人类角色跨越了幼年/少年/中年/老年四个视觉年龄阶段而需要新的成长阶段人物 id
   - "new_variant": 同一角色出现新的可复用服装/身份造型；必须分配新 id，同时填写 previous_name 和 base_character_id，后续基于原造型角色图进行图生图
...
5. 如果全局资产里已经存在同一角色身份，绝对不要因为新揭示的外貌细节输出 asset_action="new"；换装/新身份造型必须使用 new_variant，跨视觉年龄阶段才允许 new。若确实需要改同一角色同一造型，必须用 update。
...
17. 如果同一角色在本章节前后出现明显换装、身份装束变化、制服/便服切换、伪装造型、战甲/礼服/囚服等不同可复用造型，必须把换装前后的造型分别保存为不同角色资产：
   ...
   - 新造型必须 asset_action="new_variant"，分配新 id，不要用 update 覆盖旧造型。
   - name 要用稳定名称区分造型，例如“秦羽（常服）”“秦羽（战甲）”“秦羽（伪装后）”。
   - appearance 保持同一角色的核心外貌一致，clothing 分别写对应造型。
   - previous_name 必须填写作为图生图参考的已有基础造型名称；base_character_id 必须填写该基础造型的角色 id；asset_reason 写明“同一角色新增换装造型，需要独立角色资产并基于原角色图进行图生图”。
...
{{
    "characters": [
        {{
            ...
            "personality_traits": "性格特征简述",
            "asset_action": "reuse/update/new/new_variant",
            "asset_reason": "为什么复用、更新或新增",
            "previous_name": "复用、更新或换装变体时填作为参考的已有角色资产名；全新角色填空字符串",
            "base_character_id": "换装变体填写基础造型角色 id，其他情况填 null"
        }}
    ]
}}
```
...
5. 补充的新造型如果全局角色资产中不存在，则新增角色资产，asset_action="new_variant"，id 可先填 null 或临时数字，调用方会统一分配最终 id；previous_name 和 base_character_id 必须指向同一角色已有的基础造型资产；asset_reason 写明“同一角色新增换装造型，需要独立角色资产并基于原角色图进行图生图”。
...
class CharacterExtractor:
        ...
        for char in characters:
            ...
            if action in {"reuse", "update"}:
                ...
                if not isinstance(char_id, int) or char_id in used_ids or char_id <= max_previous_id:
                    char_id = next_id
                    next_id += 1
                char["asset_action"] = action if action == "new_variant" else "new"
            char["id"] = char_id
            used_ids.add(char_id)
            char.setdefault("asset_reason", "")
            char.setdefault(
                "previous_name",
                char.get("name", "") if char.get("asset_action") in {"reuse", "update"} else "",
            )
            char.setdefault("base_character_id", None)
        return characters
    ...
    def _find_costume_base_character(self, char, previous_characters, current_characters):
        """按去除造型后缀的稳定身份名，为换装资产寻找图生图基础造型。"""
        name = str(char.get("name") or "").strip()
        base_name = re.sub(r"[（(][^）)]*[）)]\s*$", "", name).strip()
        if not base_name:
            return None
        candidates = list(previous_characters or []) + list(current_characters or [])
        for candidate in candidates:
            if not isinstance(candidate, dict) or candidate is char:
                continue
            candidate_name = str(candidate.get("name") or "").strip()
            candidate_base_name = re.sub(r"[（(][^）)]*[）)]\s*$", "", candidate_name).strip()
            if candidate_base_name != base_name:
                continue
            if candidate.get("asset_action") == "new_variant":
                continue
            return candidate
        return None
