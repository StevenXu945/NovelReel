...
21. 如果某个分镜带有有效 reference_environment_id，必须检查它的 environment 是否以“基于参考场景图，”开头，并结合当前片段剧情写出新的静态取景范围、构图方位或局部细节；描述正文中不要再写 environment_id，也不要再使用“图生图变体”等重复说明。不要只写“更压抑”“更局促”“更昏暗”，不要写入角色图索引或道具图索引中的已有资产信息，也不要生成与当前片段剧情无关的新地点、新天气或新布置。
22. 如果 reference_environment_id 变体环境的前缀不正确，或缺少基于当前剧情的视角、局部细节或构图调整，且该分镜本身有 environment 字段，必须在 corrections 中返回修正后的 environment，统一以“基于参考场景图，”开头，并补充具体的静态空间、稳定布置、光线、色调、构图方位和可稳定存在的背景元素；不要改剧情和 video_prompt。
23. 如果连续超过 3 个片段复用同一环境图，且你判断后续片段需要换视角、局部细节或根据剧情发生点调整环境图，但没有可用的同场景变体环境 id，必须在 corrections 中为对应 storyboard_id 新增本章节同场景图生图变体 environment_id、reference_environment_id 和 environment。新增 environment_id 必须从当前已有最大 environment_id + 1 开始递增；reference_environment_id 必须填写被参考的原 environment_id；environment 必须以“基于参考场景图，”开头，并按剧情发生位置调整视角、局部细节或取景范围，正文不再重复 environment_id。
